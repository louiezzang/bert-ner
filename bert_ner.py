...
def crf_loss(logits, labels, mask, num_labels, mask2len):
    ...
    # <<<
    trans = tf.Variable(initializer(shape=(num_labels, num_labels)), name="transition")
...
def create_model(bert_config, is_training, input_ids, mask,
    ...
    if FLAGS.crf:
        ...
        predict, viterbi_score = tfa.text.crf_decode(
            potentials=logits,
            transition_params=trans,
